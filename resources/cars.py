...
class IdentifyCar(Resource):
    ...
    def post(self):
        ...
        new_model = load_model('cars_model.h5')
        new_model.compile(optimizer=Adam(lr=0.0001), loss='binary_crossentropy', metrics = ['accuracy'])
        #img_path = "./bmw.jpg"
        
        file = request.files['image'].read() ## byte file
        npimg = np.fromstring(file, np.uint8)
        img_read = cv2.imdecode(npimg,cv2.IMREAD_COLOR)
        
        #img = image.load_img(img_read,target_size=(224,224))
        img = cv2.resize(img_read,(224,224))
        img_array = image.img_to_array(img)
        img_batch = np.expand_dims(img_array, axis=0)
        img_preprocessed = tf.keras.applications.mobilenet.preprocess_input(img_batch)
        prediction = new_model.predict(img_preprocessed, batch_size=10, verbose =0)
        predicted_class_indices= np.argmax(prediction, axis =1)
        
        labels = [class_labels[i] for i in predicted_class_indices]
...
class DecodeVIN(Resource):
    ...
    def post(self):
        ...
        filter_txt = re.findall("\\b[A-Z0-9]{17}",result[0])
        print(result[0])
        print(filter_txt)
        
        apiString = "https://vpic.nhtsa.dot.gov/api/vehicles/decodevin/"
        apiString = apiString + filter_txt[0] +"?format=json"
        print(apiString)
